...
class NeuralBanditModel(BayesianNN):
  ...
  def train_online(self, data, num_steps, intial_cov, cov_prior, pgd_steps, pgd_freq=1):
    """Trains the network for num_steps, using the provided data.
    ...
    with self.graph.as_default():
        ...
        if step % pgd_freq == 0:
            ...
            for action, X in enumerate(cov):
                ...
                if z_i.shape[0] != 0:
                    ...
                    # #update cov with the batch
                    # for c in z_i:
                    #     A = X.dot(c)
                    #     X = X - np.outer(A,A) / (1 + A.dot(c.T))
      ...
      for action, X in enumerate(cov):
          ...
          new_precision_prior.append(np.linalg.inv(X))
          ...
          cov[action] = X
          new_precision.append(np.linalg.inv(X))

      self.times_trained += 1
      return new_precision_prior, new_precision, cov, f
